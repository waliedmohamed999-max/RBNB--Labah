...
class Experience extends Model
{
    ...
    public function listOfExperiences($data)
    {
        ...
        if (!empty($voteable)) {
            $sql->where('experience.voteable', 'on');
        }
        if ($number != -1) {
    ...
    public function getAllExperiences($data)
    {
        ...
        if (!empty($data['vote']) && $data['vote']=='true' ) {
            $sql->where('experience.voteable', 'on');
        }
    ...
    public function deleteExperienceItem($experience_id)
    {
        ...
        DB::table('voits')->where('experienceID', $experience_id)->delete();  
        DB::table('participants')->where('experienceID', $experience_id)->delete();
