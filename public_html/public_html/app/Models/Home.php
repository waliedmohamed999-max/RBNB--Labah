...
class Home extends Model
{
    ...
    public function getSearchResult($data)
    {
        ...
        $sql = DB::table($this->getTable())->selectRaw("SQL_CALC_FOUND_ROWS *");
         ...
         if(!empty($data['last-minute'])){
  
                $sql->selectRaw("COUNT(last_minute.home_id) as total_id");
                $sql->Join('last_minute', function ($join)  {
                    $join->on('home.post_id', '=', 'last_minute.home_id');
                     $join->whereRaw("last_minute.last = 1 ");
                });
                 $sql->groupBy('last_minute.home_id');
                
              
                //  $sql->Join('home_price', function ($join)  {
                //             $join->on('last_minute.home_price_id', '=', 'home_price.id');
                //         });
        ...
        if (!empty($data['offer']) || !empty($data['sm'])) {
          $has_offer=[];
         $sql2 =HomePrice::where('available', 'on')->get();
        $NowDate=date("Y-m-d H:i:s");
            foreach ($sql2 as $k => $v)
            {
              
            if($NowDate<=date('Y-m-d H:i:s', $v->end_time))
            {
              $has_offer[]=$v->home_id;}
              
            }
            $sql->whereIn('post_id', $has_offer);
            $sql->orderByDesc('home.post_id');
        }
        ...
        if (!empty($data['home-type'])) {
           ...
           if ($data['bookingType'] == 'per_hourS') {
           $sql->whereRaw("booking_type = 'per_hour'");
        }
    ...
    public function listOfHomes($data)
    {
        ...
        if (!empty($is_featured)) {
            ...
            if(!empty($data['last-minute'])){
            $sql = DB::table($this->getTable())->selectRaw("SQL_CALC_FOUND_ROWS *")->where('home.status', 'publish')->orderBy($data['orderby'], $data['order']);
    ...
    public function getAllHomes($data)
    {
      ...
      public function getById($home_id ,$last='')
    {
       
        if (!empty($last)) {
          
            $sql = DB::table($this->getTable())->selectRaw("SQL_CALC_FOUND_ROWS *");
            $sql->whereRaw("home.post_id = $home_id");
            $sql->Join('last_minute', function ($join)  {
                $join->on('home.post_id', '=', 'last_minute.home_id');
            });
            $post =$sql->first();
    ...
    public function countHome( $home_id)
    {
        return DB::table($this->getTable())->where('post_id', $home_id)->update(['count'=> DB::raw('count+1')]);
    }
    ...
    public function createHome($data = [])
    {
        ...
        $data = wp_parse_args($data, $default);
        $s=DB::table($this->getTable())->where('author',get_current_user_id())->orderBy('post_id', 'DESC')->first();
        if($s!=null&&$s->status == 'revision')
        {
        return $s->post_id;
        }else{
