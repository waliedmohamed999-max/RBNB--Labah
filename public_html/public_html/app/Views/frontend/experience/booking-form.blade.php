...
<form class="form-action" action="{{ url('add-to-cart-experience') }}"
    ...
    <?php 
        $tax_value = (float)get_option('home_tax', 0);
        $Commission = (float)get_option('blagat_tax', 0);;
        $basePrice = $post->base_price;
        if($post->use_offer == 'on'){
           $basePrice= ($post->base_price-(($post->base_price*$post->offer)/100));
        }
        $total_amount=$basePrice +(($basePrice*$Commission)/100)+((($basePrice+($basePrice*$Commission)/100)*$tax_value)/100);
    
    if ($post->booking_type == 'just_price') {
        ?>
    <div class="extra-services" style="border-top: 1px solid #fff;margin-top: 0px;padding-top: 0px;padding: 0px 10px 0px 10px;">
                <label class="d-block mb-2" style="border-bottom: 1px solid #EEE;padding-bottom: 14px;">
                    {{__('Summary')}}
                        ...
                        <div class="extra-item d-flex">
                            <span
                                class="name">{{__('Labayh Commission')}}</span>
                            <span
                                class="price ml-auto">{{$Commission }}% ({{number_format((($basePrice*$Commission)/100),2,",",".") }} ر�`ا� ) </span>
                        </div>
                                           <div class="extra-item d-flex">
                            <span
                                class="name">{{__('Tax')}} <span> ا���`�&ة ا��&ضافة (vat)</span></span>
                            <span
                                class="price ml-auto">{{ $tax_value }}% ({{ number_format(((($basePrice+($basePrice*$Commission)/100)*$tax_value)/100),2,",",".")}} ر�`ا� ) </span>
                        </div>
                               <div class="extra-item d-flex" style="border-top: 1px solid #EEE;padding-top: 15px;padding-bottom: 10px;margin-top: 15px;">
                            <span
                                class="name">{{__('Amount')}}</span>
                            <span
                                class="price ml-auto">{{ number_format($total_amount,2,",",".")}} ر�`ا�</span>
                        </div>
    ...
    @endif
    <?php
    if($post->booking_type != 'package' && $post->booking_type != 'just_price'){
    ...
    <div class="form-group mt-2">
        <input type="hidden" name="experienceID" value="{{ $post->post_id }}">
         <input type="hidden" name="base_price" value="{{ $post->base_price }}">
         <input type="hidden" name="voteable" value="{{ $post->voteable }}">
        ...
        <input type="hidden" name="bookingType"
               ...
               @if($post->subscribe_closed=='off')
        <input type="submit"
               class="btn  btn-block text-uppercase booking-now"
               name="sm"
               value="{{__('Subscribe now')}}" >
               @else
               <div 
               class="btn  btn-block text-uppercase booking-closed"
              >{{__('Subscribe Closed')}}</div>
               @endif
    </div>
