...
<div class="row">
    <div class="col-12 col-sm-6">
        <div class="form-group">
            ...
            <select name="user_role" id="user_role_edit" class="form-control wide" data-plugin="customselect">
                ...
                @foreach($roles as $key => $role)
                    <option @if($user_role == $key) selected @endif value="{{ $key }}">{{ __($role) }}</option>
                @endforeach
            </select>
        </div>
    </div>
     <div class="col-12 col-sm-6">
        <div class="form-group">
            <label for="user_last_name">{{__('Mobile')}}</label>
            <input type="text" class="form-control" id="user_mobile" class="form-control has-validation"
            data-validation="required" name="user_mobile" value="{{ $user->mobile }}">
        </div>
    </div>
</div>
<div class="form-group">
