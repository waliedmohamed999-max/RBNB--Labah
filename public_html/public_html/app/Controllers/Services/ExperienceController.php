...
class ExperienceController extends Controller
{
    ...
    public function _getSearchResult()
    {
        ...
        if ($data['total'] > 0) {
            $column = 
            // (!isset($post_data['showmap']) || (isset($post_data['showmap']) && $post_data['showmap'] == 'yes')) ? 'col-12 col-sm-6 col-md-4' :
                'col-6 col-sm-6 col-md-4 col-lg-3 col-xl-2';
            foreach ($data['results'] as $k => $item) {
                $item = $this->setup_post_data($item);
                $html .= '<div class="' . esc_attr($column) . '" style="background:#f8f4ff;border-radius: 20px;margin-bottom: 25px;">';
 ...
 if ($bookingType == 'just_price'){
     $checkAvailability['status'] =1;
            ...
            if ($checkAvailability['status'] == 0) {
                ...
                if ($experienceObject->booking_type == 'date_time') {
                    ...
                    if($experienceObject->use_offer=='on'){
                        
                         $basePrice = ($basePrice-(($basePrice*$experienceObject->offer)/100));
               ...
               elseif ($experienceObject->booking_type == 'just_price') {
                ...
                $blagatTaxData = \Cart::get_inst()->getBlagatCommission('blagat');
                if ($taxData['included'] == 'off') {
                    $taxRule = [
                         [
                                'unit' => 'tax',
                                'price' => $blagatTaxData['Commission']
                            ],
                        [
                            'unit' => 'tax',
                            'price' => $taxData['tax']
        ...
        if (hh_compare_encrypt($experienceID, $experienceEncrypt) && $startTime) {
            ...
            if ($bookingType == 'package') {
                ...
                $extra = $this->getExtraPrice($experience, $extraServices, 1);
                  ...
                  if($experience->use_offer=='on'){
                        
                        $prices['total'] = ($prices['total']-(($prices['total']*$experience->offer)/100));
                ...
                $html .= '<li class="item"><span class="title">' . __('Total') . '</span><span class="desc">' . ($total).__('Rial') . '</span></li>';
                $html .= '</div>';
            }
        }
    ...
    public function getById($experience_id, $global = false)
    {
     ...
     public function getByIdVote($experience_id, $global = false)
    {
        $experience_object = new voit();
        $post_item = $experience_object->getById($experience_id);
        if (!is_null($post_item)) {
            $post_item = $this->setup_post_data($post_item);
        }
...
//vote
    public function _voteDitailsExperience (Request $request, $id = null)
    {
        ...
        $experienceObject = $experience->getById($id);
        if (is_object($experienceObject) && user_can_edit_service($experienceObject)) {
               $Vote=Voit::where('experienceID',$id)->first();
            return view("dashboard.screens.{$folder}.services.experience.voit-ditails", ['role' => $folder, 'bodyClass' => 'hh-dashboard','Vote'=>$Vote, 'newExperience' => $id]);
        } else {
            return redirect()->to(dashboard_url('center-vote'));
        }
    }

    public function _voteDitailsExperienceAdd (Request $request, $id = null)
    {
     
          $folder = $this->getFolder();
            $expId=Voit::where('experienceID',$request->experienceID)->first();
           
           
          if (is_null($expId)) {
             
               $item=Voit::create(array_merge($request->all(),
             ['name' => serialize ($request->name) ]
            , ['start_date' => serialize ($request->start_date) ]
            , ['per' => serialize ($request->per) ]
            , ['end_date' => serialize ($request->end_date) ]
        ));
        if($request->image!=null){
            $id_media=app(\App\Controllers\MediaController::class)->_addMediaApi($request->image,get_current_user_id());
         Voit::where('id', $item->id)->update( ['image' => $id_media ]);
        }
        } else {
            
            $expId->name = serialize ($request->name);
             $expId->start_date = serialize ($request->start_date);
              $expId->end_date = serialize ($request->end_date);
                $expId->status = serialize ($request->status);
               $expId->per = serialize ($request->per);
             $expId->event_date = $request->event_date;
             
                $expId->save();
          
        
          if($request->image!=null){
              app(\App\Controllers\MediaController::class)->deleteAttachment($expId->image);
          
        $id_media=app(\App\Controllers\MediaController::class)->_addMediaApi($request->image,get_current_user_id());
         Voit::where('id', $expId->id)->update( ['image' => $id_media ]);
        }
        ...
        if (!is_null($expId) || !is_null($item)) {
            $this->sendJson([
                'status' => 1,
                'title' => __('System Alert'),
                'message' => __('Created Successfully'),
                'redirect' => dashboard_url('center-vote'),
    ...
    {
       $itemvote=Participants::where('showsub','on')->where('experienceID',$id)->get();
        $Vote=Voit::where('experienceID',$id)->first();
            $endDate=unserialize($Vote->end_date);
          $startDate=unserialize($Vote->start_date);
             $today= date("Y-m-d");
              $startVote=1;
        foreach ($startDate as $key => $value) {

            
            if ( $today <=  $Vote->event_date) {
                $date =$Vote->event_date;
                 $startVote=0;
            }
            elseif ( $today <=  $endDate[$key] ) {
                $date = $endDate[$key];
                 return view("frontend.experience.search.vote",compact('itemvote','Vote','date','startVote'));
        }
    }
    ...
    {
        $ExperienceVote=Voit::all();
        return view("frontend.experience.search.all-vote",compact('ExperienceVote'));
    }
    ...
    public function participants (Request $request)
    {
        ...
        $userID=User::where('mobile',$request->userID)->first();
      $participants = Participants::create(array_merge($request->all(), ['userID' =>$userID->id ]));
       if($request->image!=null){
            $id_media=app(\App\Controllers\MediaController::class)->_addMediaApi($request->image,get_current_user_id());
    ...
    public function _getSub ($id)
    {
          $participants = Participants::find($id);
          if (!is_null($participants)) {
              $img=get_attachment_url($participants->image );
        return [$participants,$img];
        } else { return 0 ;}
    }
    
     
     public function participantsLink (Request $request)
    {
       
  $data = request()->except(['_token','image','experienceID']);

      $participants = Participants::where('id',$request->id)->update($data);
      
         if($request->image!=null){
             app(\App\Controllers\MediaController::class)->deleteAttachment($participants->image);
            $id_media=app(\App\Controllers\MediaController::class)->_addMediaApi($request->image,get_current_user_id());
         Participants::where('id', $request->id)->update( ['image' => $id_media ]);
        }
        if (!is_null($participants)) {
            $this->sendJson([
                'status' => 1,
    ...
    public function _countVote ($id)
    {
   
      $participants = Participants::where('id',$id)->increment('count_vote');
       $data=[
        'sub_id' => $id ,
         'user_id' => get_current_user_data()->id
       ];
      DB::table('user_sub')->insert($data);
    ...
    public function _deleteVote (Request $request)
    {

      $item = Voit::where('experienceID',$request->serviceID)->delete();
    ...
    public function _centerVote(Request $request, $page = 1)
    {
        $folder = $this->getFolder();
        ...
        $status = request()->get('status');
            ...
            return view("dashboard.screens.{$folder}.services.experience.center-vote", ['role' => $folder, 'bodyClass' => 'hh-dashboard', 'allExperiences' => $allExperiences]);
    

    }
    
       public function voteEntrant( $id)
    {
        $folder = $this->getFolder();
        $allExperiences=Participants::where('experienceID',$id)->get();
            return view("dashboard.screens.{$folder}.services.experience.entrant-vote", ['role' => $folder, 'bodyClass' => 'hh-dashboard', 'allExperiences' => $allExperiences]);
    }
        ...
        if (is_null($experienceObject) || !$experienceObject || $experienceObject->status != 'publish') {
            return view('frontend.404');
             ...
             $shareComponent = \Share::page(
                'https://labayh.sa/home/'.$experience_id.'/'.$experience_name)
            ->facebook()
            ->twitter()
            ->linkedin()
            ->telegram()
            ->whatsapp()        
            ->reddit();
            return view('frontend.experience.default', compact('shareComponent'));
        }
    }
