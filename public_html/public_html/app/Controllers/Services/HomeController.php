...
class HomeController extends Controller
{
    ...
    public function getById($home_id, $global = false,$last='')
    {
        $home_object = new Home();
        // dd($last);
        $post_item = $home_object->getById($home_id,$last);
...
public function getRealPrice($post, $startTime, $endTime, $num_guest = 0 , $last='')
    {
    
        $price_model = new HomePrice();
           if ($last != '') {
                        $p = LastMinute::find($last);
                    
                    $price = $p->price;
                    }else{
        $price = $post->base_price;
        if($post->use_offer=='on'){
            $price = ($post->base_price-(($post->base_price*$post->offer)/100));
    ...
    public function getRealPriceByTime($post, $startTime, $endTime, $num_guest = 0 , $last='')
    {
    ...
    //                 $basePrice = $this->getRealPrice($homeObject, $startDate, $endDate, $numberGuest,$request->last);
    //                 }else{$basePrice = $this->getRealPrice($homeObject, $startDate, $endDate, $numberGuest);  }
    ...
    //                     $basePrice = $this->getRealPriceByTime($homeObject, $data['startTime'], $data['endTime'], $numberGuest ,$request->last);
    //                     }else{ $basePrice = $this->getRealPriceByTime($homeObject, $data['startTime'], $data['endTime'], $numberGuest);  }
...
public function _addToCartHome(Request $request, $api = false)
    {
          $role_id = null;
          $id=0;
        if(!$api){$id = get_current_user_id();}


        if ($id != 0) {
            $role_id = get_user_role($id)->id;
        }
       ...
       // if ($role_id != 2 || $api) {
        ...
        //         return [
        //             'status' => 0,
        //             'message' => 'لايمكن الحجز بعضوية الشريك '
        //         ];
        //     } else {
        //         $this->sendJson([
        //             'status' => 0,
    ...
    public function _getHomeSingle(Request $request, $home_id, $home_name = null, $last =null)
    {
       
          $homeObject = $this->getById($home_id ,true,$last);
        //   dd($homeObject);
        ...
        if (is_null($homeObject) || !$homeObject || $homeObject->status != 'publish') {
           ...
           $shareComponent = \Share::page(
                'https://labayh.sa/home/'.$home_id.'/'.$home_name)
            ->facebook()
            ->twitter()
            ->linkedin()
            ->telegram()
            ->whatsapp()        
            ->reddit();
            return view('frontend.home.default', compact('shareComponent'));
        }
    }
    ...
    public static function get_inst()
    {
      ...
      public function _lastMinute(Request $request , $page = 1)
    {
        ...
        //  strtotime('+1 day', strtotime('2021-01-03'));
        ...
        $count =0;
        $item = [] ;
       foreach ($allHomes['results'] as $key => $value) {
       
        $lastItems = $price_model->getLastMinute( $tomorrow, 'on' ,$value->post_id );
       ...
       $lastHomes = [
        ...
        // dd($lastHomes);
           return view("dashboard.screens.{$folder}.services.home.last-Minute", ['role' => $folder, 'bodyClass' => 'hh-dashboard', 'allHomes' => $lastHomes]);
 
    }

    public function _addPriceLastMinute(Request $request)
    {
        ...
        $data = [
            ...
            'start_time_last' => strtotime(date("Y-m-d" ,strtotime('tomorrow')))
        ];
        $folder = $this->getFolder();
        $home = new LastMinute();
        $newHome = $home->createLAstHome( $data);
        return redirect()->to(dashboard_url('last-Minute-home'));
  
    }
    
     public function _changeStatusLastMinute(Request $request)
    {
        ...
        $updated =  DB::table('last_minute')
              ->where('id', $id)
              ->update(['last' =>  $status ? 1 : 0]);
