<?php

namespace App\Controllers;

use App\Http\Controllers\Controller;
use App\Models\User;
use App\Models\Agreement;
use SimpleSoftwareIO\QrCode\Facades\QrCode;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\Validator;
use Sentinel;
use Illuminate\Support\Facades\DB;
class DashboardController extends Controller
{
    public function _getQRcode(Request $request)
    {
        $service_id = request()->get('serviceID');
        $service_encrypt = request()->get('serviceEncrypt');
        $service_type = request()->get('serviceType', 'home');

        if (!hh_compare_encrypt($service_id, $service_encrypt)) {
            return $this->sendJson([
                'status' => 0,
                'title' => __('System Alert'),
                'message' => __('This service does not exist')
            ]);
        }

        $serviceObject = get_post($service_id, $service_type);
        if (!is_object($serviceObject)) {
            return $this->sendJson([
                'status' => 0,
                'title' => __('System Alert'),
                'message' => __('This service does not exist')
            ]);
        }

        $service_url = get_the_permalink($serviceObject->post_id, $serviceObject->post_slug,'', $serviceObject->post_type);
        start_get_view();
        ?>
        <div class="qrcode-render">
            <?php
            echo QrCode::size(200)->generate($service_url);
            ?>
        </div>
        <?php
        $qrcode_html = end_get_view();
        return $this->sendJson([
            'status' => 1,
            'title' => __('System Alert'),
            'message' => __('Successfully get QR Code'),
            'html' => $qrcode_html
        ]);
    }

    public function _updateYourPayoutInformation(Request $request)
    {
        $user_id = request()->get('user_id');
        $user_encrypt = request()->get('user_encrypt');
        if (!hh_compare_encrypt($user_id, $user_encrypt)) {
            return $this->sendJson([
                'status' => 0,
                'title' => __('System Alert'),
                'message' => __('This user does not exist')
            ]);
        }

        $user = get_user_by_id($user_id);

        if (!$user) {
            return $this->sendJson([
                'status' => 0,
                'title' => __('System Alert'),
                'message' => __('This user does not exist')
            ]);
        }
        $payout_payment = request()->get('payout_payment');
        $payout_detail = request()->get('payout_detail');

        update_user_meta($user_id, 'payout_payment', $payout_payment);
        update_user_meta($user_id, 'payout_detail', $payout_detail);

        return $this->sendJson([
            'status' => 1,
            'title' => __('System Alert'),
            'message' => __('Updated payout information successfully')
        ]);
    }

    public function _updatePassword(Request $request)
    {
        $user_id = request()->get('user_id');
        $user_encrypt = request()->get('user_encrypt');
        if (!hh_compare_encrypt($user_id, $user_encrypt)) {
            return $this->sendJson([
                'status' => 0,
                'title' => __('System Alert'),
                'message' => __('This user does not exist')
            ]);
        }

        $user = get_user_by_id($user_id);

        if (!$user) {
            return $this->sendJson([
                'status' => 0,
                'title' => __('System Alert'),
                'message' => __('This user does not exist')
            ]);
        }

        $validator = Validator::make($request->all(), [
            'password' => ['required', 'string', 'min:6', 'confirmed'],
        ]);

        if ($validator->fails()) {
            return $this->sendJson([
                'status' => 0,
                'title' => __('System Alert'),
                'message' => $validator->errors()->first()
            ]);
        } else {
            $password = trim(request()->get('password'));
            $credentials = [
                'password' => $password,
            ];
            $user_updated = Sentinel::update($user, $credentials);
            return $this->sendJson([
                'status' => 1,
                'title' => __('System Alert'),
                'message' => __('Updated password successfully')
            ]);
        }
    }

    public function _getFontIcon(Request $request)
    {
        global $text;
        $text = request()->get('text', '');
        $text = strtolower(trim($text));
        if (empty($text)) {
            $this->sendJson(
                [
                    'status' => 0,
                    'data' => __('Not found icons')
                ]
                , true);
        }
        include public_path('fonts/fonts.php');
        include public_path('fonts/fonts-system.php');
        if (!isset($fonts) && !isset($fonts_system)) {
            $this->sendJson([
                'status' => 0,
                'data' => __('Not found icons data')
            ], true);
        }
	    $fonts_merge = [];
	    if (isset($fonts)) {
		    $fonts_merge = $fonts;
	    }
	    if (isset($fonts_system)) {
		    $fonts_merge = array_merge($fonts_merge, $fonts_system);
	    }
        $results = array_filter($fonts_merge, function ($key) {
            global $text;
            if (strpos(strtolower($key), $text) === false) {
                return false;
            } else {
                return true;
            }
        }, ARRAY_FILTER_USE_KEY);
        if (empty($results)) {
            $this->sendJson([
                'status' => 0,
                'data' => __('Not found icons')
            ], true);
        } else {
            $this->sendJson([
                'status' => 1,
                'data' => $results
            ], true);
        }
    }

    public function _updateYourAvatar(Request $request)
    {
        $user_id = request()->get('user_id');
        $user_encrypt = request()->get('user_encrypt');
        $avatar = request()->get('avatar');
        if (hh_compare_encrypt($user_id, $user_encrypt) && $user_id == get_current_user_id()) {
            $user_model = new User();
            $updated = $user_model->updateUser($user_id, ['avatar' => $avatar]);
            if (!is_null($updated)) {
                return $this->sendJson([
                    'status' => 1,
                    'title' => __('System Alert'),
                    'message' => __('Updated successfully')
                ]);
            } else {
                return $this->sendJson([
                    'status' => 0,
                    'title' => __('System Alert'),
                    'message' => __('Can not update this user. Try again!')
                ]);
            }
        } else {
            return $this->sendJson([
                'status' => 0,
                'title' => __('System Alert'),
                'message' => __('This user is invalid')
            ]);
        }
    }

    public function _updateYourProfile(Request $request)
    {
        $user_id = request()->get('user_id');
        $user_encrypt = request()->get('user_encrypt');
        $first_name = request()->get('first_name');
        $last_name = request()->get('last_name');
        $email = request()->get('email');
        $location = request()->get('location');
        $address = request()->get('address');
        $description = request()->get('description');
         $trade_name = request()->get('trade_name');
         
         if($user_encrypt == 55){
             
               $args = [
                'first_name' => $first_name,
                'last_name' => $last_name,
                'email' => $email,
                'location' => $location,
                'address' => $address,
                'description' => $description,
                'trade_name'=>$trade_name,
            ];
            $user = get_user_by_id($user_id);

            if (is_admin($user_id)) {
                $mobile = request()->get('mobile');
                $check_user = get_user_by_email($mobile);
                if ($user->mobile != $mobile && (empty($mobile) || !is_email($mobile) || $check_user)) {
                    return $this->sendJson([
                        'status' => 0,
                        'title' => __('System Alert'),
                        'message' => __('Can not use this mobile')
                    ]);
                }
                $args['mobile'] = $mobile;
            }

            $user_model = new User();
            $updated = $user_model->updateUser($user_id, $args);
               $redirect = $request->checkurl != null ?  get_referer(url('dashboard')) : url()->previous();
            if (!is_null($updated)) {
                return $this->sendJson([
                    'status' => 1,
                    'title' => __('System Alert'),
                    'message' => __('successfully registered'),
                     'redirect' => $redirect
                ]);
            } else {
                return $this->sendJson([
                    'status' => 0,
                    'title' => __('System Alert'),
                    'message' => __('Can not update this user. Try again!')
                ]);
            }
         }
             
         

       elseif(hh_compare_encrypt($user_id, $user_encrypt) && $user_id == get_current_user_id()) {
            $args = [
                'first_name' => $first_name,
                'last_name' => $last_name,
                'email' => $email,
                'location' => $location,
                'address' => $address,
                'description' => $description,
                'trade_name'=>$trade_name,
            ];
            $user = get_user_by_id($user_id);

            if (is_admin($user_id)) {
                $mobile = request()->get('mobile');
                $check_user = get_user_by_email($mobile);
                if ($user->mobile != $mobile && (empty($mobile) || !is_email($mobile) || $check_user)) {
                    return $this->sendJson([
                        'status' => 0,
                        'title' => __('System Alert'),
                        'message' => __('Can not use this mobile')
                    ]);
                }
                $args['mobile'] = $mobile;
            }

            $user_model = new User();
            $updated = $user_model->updateUser($user_id, $args);
               $redirect = $request->checkurl != null ?  get_referer(url('dashboard')) : url()->previous();
            if (!is_null($updated)) {
                return $this->sendJson([
                    'status' => 1,
                    'title' => __('System Alert'),
                    'message' => __('Updated successfully'),
                     'redirect' => $redirect
                ]);
            } else {
                return $this->sendJson([
                    'status' => 0,
                    'title' => __('System Alert'),
                    'message' => __('Can not update this user. Try again!')
                ]);
            }
        } else {
            return $this->sendJson([
                'status' => 0,
                'title' => __('System Alert'),
                'message' => __('This user is invalid')
            ]);
        }
    }

    public function _getProfile()
    {
        $folder = $this->getFolder();
        return view("dashboard.screens.{$folder}.profile", ['role' => $folder, 'bodyClass' => 'hh-dashboard']);
    }
    
      public function _getAgreement($user_id = null)
    {
     
        $folder = $this->getFolder();
        if ($user_id != null ) {
            $Agreement = checkAgreement($user_id);
            return view("dashboard.screens.{$folder}.agreement", ['role' => $folder, 'bodyClass' => 'hh-dashboard' , 'Agreement' => $Agreement]);
        }
        return view("dashboard.screens.{$folder}.agreement", ['role' => $folder, 'bodyClass' => 'hh-dashboard']);
    }

    public function _getAgreementItem(Request $request)
    {
        $user_id = request()->get('userID');
      
        $Agreement = checkAgreement($user_id);
        $user = get_user_by_id($user_id);
        if (!$user) {
            return $this->sendJson([
                'status' => 0,
                'title' => __('System Alert'),
                'message' => __('This user does not exist')
            ]);
        }
        return $this->sendJson([
            'status' => 1,
            'html' => view('dashboard.components.agreement-edit-form', ['Agreement' => $Agreement , 'user' => $user])->render()
        ]);
    }

  public function _getPaymentItem(Request $request)
    {
        $user_id = request()->get('userID');
      
        return $this->sendJson([
            'status' => 1,
            'html' => view('dashboard.components.payment-show', ['user_id' => $user_id])->render()
        ]);
    }
    public function _contractApproval(Request $request)
    {
  
      $validator = Validator::make($request->all(), [
            'ID_number' => ['required' ],
            'name' => ['required' ],
            'email' => ['required'],
            'address' => ['required'],
        ]);
        if ($validator->fails()) {
            return $this->sendJson([
                'status' => 0,
                'title' => __('System Alert'),
                'message' =>__('Please complete the data')
            ]);
        }else{
           
            $user_id = request()->get('user_id');
            $ID_number = request()->get('ID_number');
            $status = 0;
     
            $args = [
             'user_id' => $user_id,
             'ID_number' => $ID_number,
             'status' => $status,
         ];
     
            $agreement_model = new Agreement();
            $create = $agreement_model->createAgreement($args);
            if (!is_null($create)) {
                return $this->sendJson([
                    'status' => 1,
                    'title' => __('System Alert'),
                       'reload' => true,
                    'message' => __('Created successfully')
                ]);
            } else {
                return $this->sendJson([
                    'status' => 0,
                    'title' => __('System Alert'),
                    'message' => __('The contract was not approved. Try again!')
                ]);
            }
        }

    }

    public function _getFavAds($type)
    { $folder = $this->getFolder();
        $id=[];
        $ads= DB::table('favourite')
        ->where('phone_faved_user', get_current_user_id())
        ->where('type',$type)->get();
        foreach($ads as $ad)
        {
         array_push($id,$ad->id_ads);
        }
        $results= DB::table($type)
        ->whereIn('post_id',$id)->get();
       
         $count = DB::select("SELECT FOUND_ROWS() as `row_count`")[0]->row_count;
            $allHomes= [
                'total' => $count,
                'results' => $results
            ];
       return view("dashboard.screens.{$folder}.services.home.favourite", ['role' => $folder,'type'=>$type, 'bodyClass' => 'hh-dashboard', 'allHomes' => $allHomes]);

       
    }
    public function _removeFavAds($type,$id){
        DB::table('favourite')
        ->where('phone_faved_user', get_current_user_id())
        ->where('id_ads', $id)
        ->where('type',$type)->delete();
        
    }
    public function _addFavAds($type,$id){
        $data=['isFav'=>'1','id_ads'=>$id,'phone_faved_user'=> get_current_user_id(),'type'=>$type];
        
        DB::table('favourite')->insertGetId($data);
      
    }
    public function _updateAgreement(Request $request)
    {
     
        $id = request()->get('ID');
        $status = request()->get('val');
        $user_id = request()->get('userID');
     if($status == 1){
          $user_model = new User();
        $update = $user_model->updateUserRole($user_id, 2);
           $args = [
            'request' => 'request_a_partner',
        ];
        $user_model->updateUser($user_id, $args);
     }else{
          $user_model = new User();
        $update = $user_model->updateUserRole($user_id, 3);
           $args = [
            'request' => 'request_a_customer',
        ];
        $user_model->updateUser($user_id, $args);
     }
       

        $updated =  Agreement::
              where('id', $id)
              ->update(['status' =>  $status ? 1 : 0]);

        if (!is_null($updated)) {
            $this->sendJson([
                'status' => 1,
                'title' => __('System Alert'),
                'message' => __('Updated successfully'),
                'reload' => true
            ], true);
        } else {
            $this->sendJson([
                'status' => 0,
                'title' => __('System Alert'),
                'message' => __('Can not update this range'),
                'reload' => true
            ], true);
        }
    }
    


    public function index()
    {
        $folder = $this->getFolder();
        return view("dashboard.screens.{$folder}.dashboard", ['role' => $folder, 'bodyClass' => 'hh-dashboard']);
    }

}
