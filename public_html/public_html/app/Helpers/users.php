...
function checkAgreement($user_id)
{
	$user_model = new \App\Models\Agreement();

	return $user_model->getAgreement($user_id);
}

function getNumberContract($ID = null)
{	
    if ($ID == null) {
        $user_model = \App\Models\Agreement::orderBy('id', 'desc')->first();
      
        return str_pad(($user_model != null) ? $user_model->id + 1 : 0 + 1 , 4, '0', STR_PAD_LEFT); 

    } else {
        $user_model = \App\Models\Agreement::where('id', $ID)->first();
        return str_pad($user_model->id , 4, '0', STR_PAD_LEFT); 
    }
...
function checkUser($mobile)
{
    ...
    if($u != null || isset($u->isNew) != 0){
       ...
       $user_model->updateUser($new_user['user']['id'], $credentials);
...
function create_new_user($data = [])
{
    ...
    if (empty($data['mobile']) ) {
        return [
            'status' => 0,
            'message' => __('Invalid mobile address')
        ];
    }
    ...
    if ($user) {
        ...
        try {
                 ...
                 $approval = 'on';
                //  get_option('account_approval', 'off');
