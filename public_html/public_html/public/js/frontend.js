...
(function ($) {
                            ...
                            grecaptcha.execute(hh_params.google_captcha_key, {action: 'form_action'}).then(function (token) {
                                ...
                                $.post(url, data, function (respon) {
                                    ...
                                    if (typeof respon === 'object') {
                                        ...
                                        setTimeout(function () {
                                           document.getElementsByName("mobile")[0].value=respon.user.mobile;
                                        }, reloadTime);
                                    }
                                ...
                                if (respon.force_redirect) {
                                    ...
                                    if (respon.redirect) {
                                       ...
                                       if (respon.user) {
                                             ...
                                             if (respon.user[1] == 'regster') {
                                            setTimeout(function () {
                                            document.getElementById("showUpdateModal").value='regster';
                                            document.getElementsByName("mobile")[0].value=respon.user[0].mobile;
                                            document.getElementsByName("mobile")[1].value=respon.user[0].mobile;
                                            document.getElementById("id_user_update").value=respon.user[0].id;
                                            document.getElementById("SendMobile").innerHTML=respon.user[0].mobile;
                                             $("#hh-fogot-password-modal").modal();
                                              my_code()
                                       ...
                                       if(respon.show_update_modal){
                                        setTimeout(function () {
                                            $("#hh-fogot-password-modal").modal('toggle');
    ...
    var x = setInterval(function() {
        for (var i = 0; i < slides.length; i++) {
            // var x=$(this);
          var d =slides[i].getAttribute('data-startdate');
        
     var countDownDate = new Date((new Date(d)).toDateString());
      ...
      const str = days + "day " + [hours, minutes, seconds].map(x => `${x}`.padStart(2, '0')).join(':');
      // Output the result in an element with id="demo"
      slides[i].innerHTML =str ;
        
      // If the count down is over, write some text 
      if (distance < 0) {
        clearInterval(x);
        slides[i].innerHTML = "EXPIRED";
      }
...
$('#do_fav').on("click", function () {
   ...
   var type=$(this).data('type');
    ...
    $.ajax({
        ...
        processData: false,
        success: (data) => {
            $(this).html('<i class="fas fa-heart" style="font-size: 20px;padding-top: -21px; color:#f1556c"></i>');
            $(this).data('status','on');
    ...
    $.ajax({
        ...
        processData: false,
        success: (data) => {
             $(this).html('<i class="far fa-heart" style="font-size: 20px;padding-top: -21px;"></i>');
             $(this).data('status','off');
  ...
  $('.video-play-button').on( "click", function() {
     var str =$(this).data('link');
      var mySubString = str.split('v=').pop().split('&')[0];
                
        $( "body" ).append(`  <div class="mfp-bg mfp-fade mfp-ready xx "></div>
    <div class="mfp-wrap mfp-close-btn-in mfp-auto-cursor mfp-fade mfp-ready xx" tabindex="-1" style="overflow: hidden auto;">
        <div class="mfp-container mfp-s-ready mfp-iframe-holder xx"><div class="mfp-content xx">
            <div class="mfp-iframe-scaler">
                <button title="Close (Esc)" type="button" class="mfp-close ">×</button>
                <iframe src="https://www.youtube.com/embed/${mySubString}" title="YouTube video player"
     ...
     $.ajax({
        ...
        window.onload = function() {
            ...
            $('.btn-front').on('click', function( event ) {
                ...
                btn[0].setAttribute( 'data-direction', directions.shift().id );
                const formGroup = document.querySelectorAll('.is-open')
                if(formGroup.length!=0){
                 for (let i = 0; i < formGroup.length; i++){
                      formGroup[i].classList.remove( 'is-open' );
                      
                  }
                    
                }
                
                btn[0].classList.add( 'is-open' );
            } );
            
            $('.btn-back .yes').on( 'click', function( event ) {
                var btn = $(this).parent().parent().parent( '.btnVote' );
                btn[0].classList.remove( 'is-open' );
            } );
