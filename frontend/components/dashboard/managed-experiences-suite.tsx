...
type SortMode = "newest" | "price_asc" | "price_desc" | "capacity_desc";
...
const categoryTabs = [
  ...
  { value: "experiences", label: "التجارب" },
...
const toolLinks: Array<{ href: string; label: string; icon: LucideIcon }> = [
  { href: "/dashboard/services/experiences/center-vote", label: "مركز التصويت", icon: Star },
  { href: "/dashboard/services/experiences/reviews", label: "المراجعات", icon: MessageSquareText },
  { href: "/dashboard/services/experiences/types", label: "التصنيفات", icon: Tags },
  { href: "/dashboard/services/experiences/languages", label: "اللغات", icon: Languages },
...
function ExperienceImage({ item, className }: { item: BridgeManagedServiceItem; className: string }) {
...
function categoryOf(item: BridgeManagedServiceItem) {
  const text = `${item.type_label} ${item.title}`.toLowerCase();
  if (text.includes("مؤتمر") || text.includes("conference")) return "conferences";
  if (text.includes("فعالية") || text.includes("event")) return "events";
  return "experiences";
}

export function ManagedExperiencesSuite({ managed, initialCategory = "all" }: ManagedExperiencesSuiteProps) {
  ...
  const stats = useMemo(() => {
    ...
    const capacity = items.reduce((sum, item) => sum + Number(item.guests ?? 0), 0);
    return { total, published, pending, avgPrice, capacity };
  }, [items]);
  ...
  const filteredItems = useMemo(() => {
    ...
    return [...result].sort((a, b) => {
      ...
      if (sort === "capacity_desc") return Number(b.guests ?? 0) - Number(a.guests ?? 0);
  ...
  async function submitAction(item: BridgeManagedServiceItem, action: "status" | "duplicate" | "delete", body?: unknown) {
    ...
    const response = await secureFetch(`/api/v1/dashboard/services/experiences/${item.id}/${action}`, {
      method: "POST",
  ...
  function bulkDelete() {
    ...
    const confirmed = window.confirm(`سيتم حذف ${selectedIds.length.toLocaleString("ar-SA")} عنصر. هل تريد المتابعة؟`);
    if (!confirmed) return;
    ...
    startTransition(async () => {
      ...
      setSelectedIds([]);
      setToast("تم حذف العناصر المحددة.");
    });
  }

  function exportCsv(rows = filteredItems) {
    const csvRows = [
      ["id", "title", "status", "category", "price", "capacity", "public_url"],
      ...rows.map((item) => [item.id, item.title, item.status, categoryOf(item), item.price, item.guests, publicExperiencePath(item)]),
    ];
    ...
    link.href = url;
    link.download = `experiences-${Date.now()}.csv`;
    link.click();
  ...
  return (
    <div className="space-y-6 bg-[#F7F8FA] pb-10" dir="rtl">
      {toast ? <div className="fixed left-6 top-6 z-[70] rounded-2xl border border-emerald-100 bg-emerald-50 px-5 py-3 text-sm font-bold text-emerald-700 shadow-lg">{toast}</div> : null}
      ...
      <section className="rounded-3xl border border-gray-100 bg-white p-4 shadow-sm">
        <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-6">
          {toolLinks.map((item) => (
            <Link key={item.href} href={item.href} className="inline-flex min-h-12 items-center justify-center gap-2 rounded-2xl bg-[#1a1f36] px-4 py-3 text-center text-sm font-bold text-white transition hover:bg-[#FF385C]">
          ...
          <div className="flex flex-col gap-5 xl:flex-row xl:items-start xl:justify-between">
            <div>
              <div className="inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/10 px-3 py-1 text-xs font-bold text-white/80">
                <Ticket className="size-4 text-[#FF385C]" />
                مركز إدارة التجارب والفعاليات
              </div>
              <h1 className="mt-4 text-3xl font-black">التجارب والفعاليات</h1>
              <p className="mt-2 max-w-2xl text-sm leading-7 text-white/70">
                إدارة التجارب، الرحلات، الفعاليات، والمؤتمرات من لوحة تشغيل واحدة مع أدوات فرز وتصدير ومتابعة.
              </p>
            </div>
            <div className="flex flex-wrap gap-2">
              ...
              <Link href="/dashboard/services/experiences/new" className="inline-flex items-center gap-2 rounded-xl bg-[#FF385C] px-4 py-2 text-sm font-bold text-white">
      ...
      <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        <StatCard icon={Ticket} label="إجمالي العناصر" value={stats.total.toLocaleString("ar-SA")} hint="تجارب وفعاليات ومؤتمرات" />
        <StatCard icon={CheckCircle2} label="منشورة الآن" value={stats.published.toLocaleString("ar-SA")} hint="ظاهرة في الواجهة" />
        <StatCard icon={Users} label="السعة الكلية" value={stats.capacity.toLocaleString("ar-SA")} hint="إجمالي عدد الأشخاص" />
        <StatCard icon={Sparkles} label="متوسط السعر" value={formatMoney(stats.avgPrice)} hint="متوسط السعر للفرد" />
      </section>

      <section className="rounded-3xl border border-gray-100 bg-white p-4 shadow-sm lg:p-5">
        <div className="mb-4 flex flex-wrap gap-2">
          {categoryTabs.map((tab) => (
            <button key={tab.value} onClick={() => setCategory(tab.value)} className={`rounded-full px-4 py-2 text-sm font-bold transition ${category === tab.value ? "bg-[#1a1f36] text-white" : "border border-gray-200 bg-white text-gray-600"}`}>
              {tab.label}
            </button>
          ))}
        </div>
        <div className="grid gap-3 xl:grid-cols-[1fr_180px_180px_auto_auto]">
          <label className="relative">
            ...
            <input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="ابحث بالعنوان أو رقم التجربة..." className="w-full rounded-2xl border border-gray-200 bg-white py-3 pe-11 ps-4 text-sm outline-none transition focus:border-[#FF385C] focus:ring-2 focus:ring-[#FF385C]/20" />
          </label>
          ...
          <select value={sort} onChange={(event) => setSort(event.target.value as SortMode)} className="rounded-2xl border border-gray-200 bg-white px-4 py-3 text-sm font-bold text-gray-700">
            ...
            <option value="capacity_desc">الأكبر سعة</option>
          </select>
        ...
        {selectedIds.length ? (
          <div className="mt-4 flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-[#FF385C]/15 bg-[#FF385C]/5 px-4 py-3">
            <div className="text-sm font-black text-[#1a1f36]">تم تحديد {selectedIds.length.toLocaleString("ar-SA")} عنصر</div>
            <div className="flex flex-wrap gap-2">
      ...
      <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_320px]">
        <section className="rounded-3xl border border-gray-100 bg-white p-4 shadow-sm lg:p-5">
          <div className="mb-5">
            <h2 className="text-xl font-black text-[#1a1f36]">قائمة التجارب والفعاليات</h2>
            <p className="mt-1 text-sm text-gray-500">عرض {filteredItems.length.toLocaleString("ar-SA")} من أصل {items.length.toLocaleString("ar-SA")} عنصر</p>
          </div>

          {filteredItems.length === 0 ? (
            <div className="rounded-3xl border border-dashed border-gray-200 p-12 text-center">
              <Ticket className="mx-auto size-16 text-gray-300" />
              <h3 className="mt-4 text-lg font-black text-gray-700">لا توجد عناصر مطابقة</h3>
              <p className="mt-2 text-sm text-gray-400">جرّب تغيير البحث أو الفلتر.</p>
            </div>
          ) : view === "grid" ? (
            <div className="grid gap-4 md:grid-cols-2">
              {filteredItems.map((item) => (
                <ExperienceCard key={item.id} item={item} selected={selectedIds.includes(item.id)} pending={pendingId === item.id} onSelect={() => toggleSelected(item.id)} onOpen={() => setActiveItem(item)} onAction={submitAction} />
              ))}
            </div>
          ) : (
            <div className="overflow-hidden rounded-3xl border border-gray-100">
              ...
              <div className="divide-y divide-gray-100">
                {filteredItems.map((item) => (
                  <ExperienceRow key={item.id} item={item} selected={selectedIds.includes(item.id)} pending={pendingId === item.id} onSelect={() => toggleSelected(item.id)} onOpen={() => setActiveItem(item)} onAction={submitAction} />
                ))}
              </div>
            </div>
          )}
        ...
        <aside className="space-y-4">
          <section className="rounded-3xl border border-gray-100 bg-white p-5 shadow-sm">
            ...
            <div className="mt-4 grid gap-2">
              {toolLinks.map((item) => <QuickLink key={item.href} {...item} />)}
            </div>
          </section>
          <section className="rounded-3xl border border-gray-100 bg-white p-5 shadow-sm">
      ...
      {activeItem ? <ExperienceDrawer item={activeItem} pending={pendingId === activeItem.id} onClose={() => setActiveItem(null)} onAction={submitAction} /> : null}
    </div>
  );
}

function ExperienceCard({ item, selected, pending, onSelect, onOpen, onAction }: ExperienceItemProps) {
  return (
    <article className="group overflow-hidden rounded-3xl border border-gray-100 bg-white shadow-sm transition hover:-translate-y-0.5 hover:shadow-md">
      <ExperienceImage item={item} className="aspect-[16/10] w-full" />
      <div className="space-y-4 p-4">
        ...
        <div>
          ...
          <p className="mt-1 text-sm text-gray-500">{item.type_label || "تجربة"} • #{item.id}</p>
        </div>
        <div className="flex flex-wrap gap-2 text-xs text-gray-500">
          <span className="inline-flex items-center gap-1 rounded-full bg-gray-50 px-3 py-1"><Users className="size-3" />{item.guests} شخص</span>
          <span className="inline-flex items-center gap-1 rounded-full bg-gray-50 px-3 py-1"><CalendarDays className="size-3" />جدولة</span>
          <span className="inline-flex items-center gap-1 rounded-full bg-gray-50 px-3 py-1"><Ticket className="size-3" />تذاكر</span>
        </div>
        <div className="flex items-center justify-between border-t border-gray-100 pt-4">
          ...
          <div className="flex gap-2">
            ...
            <Link href={`/dashboard/services/experiences/${item.id}/edit`} className="rounded-xl bg-[#1a1f36] px-4 py-2 text-xs font-bold text-white">تعديل</Link>
          </div>
        </div>
...
function ExperienceRow({ item, selected, pending, onSelect, onOpen, onAction }: ExperienceItemProps) {
  return (
    <article className="grid gap-4 p-4 transition hover:bg-gray-50 lg:grid-cols-[44px_1.4fr_120px_120px_120px_150px] lg:items-center">
      ...
      <div className="flex min-w-0 gap-3">
        <ExperienceImage item={item} className="size-16 shrink-0 rounded-2xl" />
        <div className="min-w-0">
          ...
          <div className="mt-1 text-xs text-gray-500">#{item.id} • {item.type_label || "تجربة"}</div>
        </div>
      </div>
      {statusBadge(item.status, item.status_label)}
      <span className="text-sm font-bold text-gray-600">{Number(item.guests || 0).toLocaleString("ar-SA")} شخص</span>
      <span className="text-sm font-black text-[#1a1f36]">{formatMoney(item.price)}</span>
      <div className="flex gap-2">
        <button onClick={onOpen} className="rounded-xl border border-gray-200 p-2 text-gray-600"><Eye className="size-4" /></button>
        <Link href={`/dashboard/services/experiences/${item.id}/edit`} className="rounded-xl bg-[#1a1f36] px-3 py-2 text-xs font-bold text-white">تعديل</Link>
...
function ActionBar({ item, pending, onAction }: Pick<ExperienceItemProps, "item" | "pending" | "onAction">) {
  const [nextStatus, setNextStatus] = useState("");
  return (
    <div className="grid gap-2 border-t border-gray-100 pt-4 sm:grid-cols-2">
      <Link href={publicExperiencePath(item)} className="inline-flex items-center justify-center gap-2 rounded-xl border border-gray-200 px-3 py-2 text-xs font-bold text-gray-700"><ExternalLink className="size-4" />عرض</Link>
      <button disabled={pending} onClick={() => onAction(item, "duplicate")} className="inline-flex items-center justify-center gap-2 rounded-xl border border-gray-200 px-3 py-2 text-xs font-bold text-gray-700 disabled:opacity-50"><Copy className="size-4" />تكرار</button>
      ...
      <button disabled={!nextStatus || pending} onClick={() => onAction(item, "status", { status: nextStatus })} className="rounded-xl bg-[#1a1f36] px-3 py-2 text-xs font-bold text-white disabled:opacity-50">تطبيق</button>
    </div>
  );
}
...
function ExperienceDrawer({ item, pending, onClose, onAction }: Pick<ExperienceItemProps, "item" | "pending" | "onAction"> & { onClose: () => void }) {
  return (
    <div className="fixed inset-0 z-50 bg-[#1a1f36]/40 backdrop-blur-sm" onClick={onClose}>
      <aside className="ms-auto h-full w-full max-w-[520px] overflow-y-auto bg-white shadow-2xl" onClick={(event) => event.stopPropagation()}>
        <div className="sticky top-0 z-10 flex items-start justify-between gap-4 border-b border-gray-100 bg-white/95 p-5 backdrop-blur">
          <div>
            <div className="text-xs font-bold text-gray-400">ملف العنصر #{item.id}</div>
        ...
        <div className="p-5">
          <ExperienceImage item={item} className="aspect-video w-full rounded-3xl" />
          <div className="mt-5 grid grid-cols-2 gap-3">
            <InfoBox label="الحالة" value={item.status_label || item.status} />
            <InfoBox label="السعر" value={formatMoney(item.price)} />
            <InfoBox label="السعة" value={`${Number(item.guests || 0).toLocaleString("ar-SA")} شخص`} />
            <InfoBox label="النوع" value={item.type_label || "تجربة"} />
          </div>
          <div className="sticky bottom-0 mt-6 flex flex-wrap gap-2 border-t border-gray-100 bg-white py-5">
            <Link href={`/dashboard/services/experiences/${item.id}/edit`} className="inline-flex items-center gap-2 rounded-2xl bg-[#FF385C] px-4 py-3 text-sm font-bold text-white"><Edit3 className="size-4" />تعديل</Link>
            <Link href={publicExperiencePath(item)} className="inline-flex items-center gap-2 rounded-2xl border border-gray-200 px-4 py-3 text-sm font-bold text-gray-700"><ExternalLink className="size-4" />عرض</Link>
            ...
            <button disabled={pending} onClick={() => window.confirm("هل تريد حذف هذا العنصر؟") && onAction(item, "delete")} className="inline-flex items-center gap-2 rounded-2xl bg-red-500 px-4 py-3 text-sm font-bold text-white disabled:opacity-50"><Trash2 className="size-4" />حذف</button>
          </div>
        </div>
      </aside>
    </div>
