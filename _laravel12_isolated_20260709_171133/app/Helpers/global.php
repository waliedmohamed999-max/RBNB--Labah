...
function checkEvent($experienceID)
{
    $check=   DB::table('voits')->where('experienceID' , $experienceID )->first();
    return $check;
}

 function checkVote($Subid)
{
    $check=   DB::table('user_sub')->where('sub_id' , $Subid )->where('user_id' , get_current_user_data()->id)->first();
    return $check;
}
...
function get_edit_link()
{
    ...
    if (isset($params['home_name'])) {
        return dashboard_url('services/homes/' . $post->post_id . '/edit');
    }
    if (isset($params['experience_name'])) {
        return dashboard_url('services/experiences/' . $post->post_id . '/edit');
    }
    if (isset($params['car_name'])) {
        return dashboard_url();
    }
    if (isset($params['post_slug'])) {
        return dashboard_url('posts/' . $post->post_id . '/edit');
    }
    if (isset($params['page_slug'])) {
        return dashboard_url('pages/' . $post->post_id . '/edit');
    }
...
function dashboard_url($url = '', $id = '', $page = '')
{
    ...
    if (!empty($page)) {
        $path .= '/' . trim((string) $page, '/');
    }

    return premium_frontend_url('dashboard' . ($path ? '/' . $path : ''));
}

function premium_frontend_url($path = '')
{
    $base = rtrim(config('app.premium_frontend_url', 'http://localhost:3000'), '/');
    $normalizedPath = empty($path) ? '' : '/' . ltrim($path, '/');
...
function is_fav($id,$type){
     $ads= DB::table('favourite')
        ->where('phone_faved_user', get_current_user_id())
        ->where('id_ads', $id)
    ...
    switch ($post_type) {
        ...
        case 'post':
            ...
            break;
             ...
             case 'voit':
                  $post = \App\Controllers\Services\ExperienceController::get_inst()->getByIdVote($post_id, $global);
            break;
    }
...
function get_the_permalink($post_id, $post_slug = '', $lastMenute = '',$type = 'home')
{
     if (empty($post_slug)) {
    ...
    if (!empty($post_slug)) {
        ...
        return url($post_info['slug'] . '/' . $post_id . '/' . $post_slug. '/' . $lastMenute);
    }
    return url('/');
}
...
function get_attachment_url($attachment_id, $size = 'full', $default = true)
{
    ...
    if (is_object($attachment)) {
        ...
        if (file_exists($media_path)) {
            ...
            $isLocalHost = in_array($appUrlHost, $localHosts, true);

            if (!$isLocalHost && !app()->runningInConsole()) {
                $isLocalHost = in_array(request()->getHost(), $localHosts, true);
            }

            if (\App::environment('production_ssl') && !$isLocalHost) {
...
function BlagatCommission($type=null){
    
    $sql = DB::table('booking')->selectRaw("sum(total) as total")->where('status', 'completed');
     if($type!=null){
         
          $sql->where('service_type', $type);
     }
...
function PBooking($user_id,$type=null){
    
    $sql = DB::table('booking')->selectRaw("sum(total) as total")->where('status', 'completed')->where('owner',$user_id);
...
function PaymentsBlagat($user_id=null){
    
    $sql = DB::table('payout')->selectRaw("sum(amount) as amount")->where('status', 'completed');
    if($user_id!=null){
...
function PaymentsRemaining($user_id=null){
    
    $sql = DB::table('payout')->selectRaw("sum(amount) as amount")->where('status', 'pending');
    if($user_id!=null){
        $sql->where('user_id', $user_id);
